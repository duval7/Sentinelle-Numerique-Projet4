...
async function handleJob(job) {
  ...
  try {
    ...
    if (claims.length === 0) {
      return await saveReport(jobId, { confidenceScore: 0, verdict: "UNKNOWN", claimBreakdown: [] });
    }
    ...
    await saveReport(jobId, { confidenceScore: null, verdict: "ERROR", error: err.message, claimBreakdown: [] });
  }
}
...
async function extractClaims(text) {
  try {
    const res = await axios.post(`${NLP_URL}/extract`, { text }, { timeout: 15000 });
    return res.data.claims || [];
  } catch {
    // NLP service not yet connected — return a stub claim for testing
    return [{ text: "Stub claim", entities: [], numericalValue: null, credibilityScore: 0 }];
  }
}

async function verifyClaim(claim) {
  const q = claim.entities.length > 0 ? claim.entities.join(" ") : claim.text.slice(0, 80);
...
async function saveReport(jobId, report) {
  // TODO Sprint 3: persist to MongoDB
  console.log(`[Engine] Report saved for ${jobId}:`, JSON.stringify(report, null, 2));
}
